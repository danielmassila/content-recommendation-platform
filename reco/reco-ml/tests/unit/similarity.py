...
def test_cosine_similarity_no_common_item_is_zero():
    ...
    assert compute_user_cosine_similarity(1, 2, by_user, sim_cache={}) == 0.0


def test_cosine_similarity_missing_user_is_zero():
    by_user = {1: {10: 4.0}}
    assert compute_user_cosine_similarity(1, 999, by_user, sim_cache={}) == 0.0
...
def test_cosine_similarity_with_common_item():
    ...
    assert compute_user_cosine_similarity(1, 2, by_user, sim_cache={}) == pytest.approx(
        1.0
    )  # v = k*u so cos = 1
